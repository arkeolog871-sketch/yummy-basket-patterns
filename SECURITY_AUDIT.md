...
The wrapper previously allowed mixed content, unrestricted top-level navigation, and granted geolocation to any requesting origin. It now rejects cleartext/mixed content, disables WebView debugging, restricts geolocation to the production host, rejects TLS errors, disables `file://` access, and opens non-app destinations externally. Third-party cookies remain **enabled** so Google OAuth cookies set in Chrome Custom Tabs can complete the `/auth` callback inside the WebView. Content access remains enabled for gallery/camera `content://` URIs.
...
- OTPs are 6-digit codes generated by the application server, stored only as SHA-256 hashes in `email_otp_guard`, expire after 10 minutes, and are limited by cooldown/hourly/failed-attempt controls. Matching codes are consumed before a session is created.
