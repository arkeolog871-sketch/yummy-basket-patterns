...
- [FIXED 2026-09-01] Live database re-check found this checklist's own "authenticated order writes are revoked" claim was **not actually true in production**: `orders`/`order_items` still granted `authenticated` direct `INSERT`, letting any logged-in customer bypass `place_customer_order`'s price/stock/minimum-order checks with a forged Supabase client insert. Closed directly against the live database and recorded in `20260901150000_grant_hardening.sql`: dropped the permissive insert-own policies, revoked `INSERT`/`DELETE`/`TRUNCATE` from `anon`/`authenticated` on both tables, and revoked all `anon` grants schema-wide down to `SELECT` on the 7 tables with an actual public-read policy. Also revoked `TRUNCATE` from `authenticated` everywhere (never legitimate, not RLS-filterable). Lesson: this file's `[PASS]` marks reflect intent in code/migrations, not a live grant audit — verify against `information_schema.role_table_grants` / `pg_policies` on the connected project before trusting them again.
