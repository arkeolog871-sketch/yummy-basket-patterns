...
BEGIN
  IF NEW.end_date <= now() THEN NEW.is_active := false; END IF;
  RETURN NEW;
END; $$;
...
RETURNS integer LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  WITH updated AS (
    UPDATE public.advertisements SET is_active = false
...
RETURNS TABLE (
  id uuid, title text, image_url text,
  action_type public.advertisement_action_type,
  action_value text, display_order integer
...
BEGIN
  IF p_type = 'impression' THEN
    UPDATE public.advertisements SET impression_count = impression_count + 1
    WHERE id = p_id AND is_active = true AND start_date <= now() AND end_date > now();
  ELSIF p_type = 'click' THEN
    UPDATE public.advertisements SET click_count = click_count + 1
    WHERE id = p_id AND is_active = true AND start_date <= now() AND end_date > now();
  ELSE
