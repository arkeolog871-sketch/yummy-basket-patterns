...
const audienceSchema = z.enum(["all", "all_vendors", "all_customers", "restaurant", "user"]);
...
export const previewFounderNotificationAudience = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) =>
    runServerFn(async () => {
      ...
      const { previewFounderAudience } = await import("./notifications.server");
      return previewFounderAudience({
        target: data.target,
...
export const listFounderNotificationBroadcasts = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) =>
    runServerFn(async () => {
      const { assertFounder } = await import("./founder.server");
      await assertFounder(context.supabase, context.userId, context.claims as never);
      const { listFounderBroadcasts } = await import("./notifications.server");
      const items = await listFounderBroadcasts(50);
      return { items };
    }),
  );
