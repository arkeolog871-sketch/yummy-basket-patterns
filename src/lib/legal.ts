...
export const LEGAL_DOCUMENTS: Record<LegalDocId, LegalDocument> = {
  ...
  kvkk: {
    ...
    paragraphs: [
      ...
      "Sesli asistan: mikrofon yalnızca kayıt düğmesine bastığınızda açılır, arka planda dinleme yapılmaz. Ses kaydı metne çevrilmek üzere yapay zekâ hizmet sağlayıcısına aktarılır ve saklanmaz; yazışma metni giriş yapmış kullanıcılar için hesaba bağlı olarak tutulur. Hukuki sebep, özelliği kullanmanız hâlinde KVKK m. 5/1 açık rızadır; asistanı kullanmamak hizmetin diğer bölümlerini etkilemez.",
