...
export const listRestaurants = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return cachedPublicQuery(`listRestaurants:${JSON.stringify(data)}`, async () => {
...
export const listCategories = createServerFn({ method: "GET" }).handler(async () => {
  const { cachedPublicQuery } = await import("./public-cache.server");
  return cachedPublicQuery("listCategories", async () => {
    const { createPublicClient } = await import("./catalog.server");
    const supabase = createPublicClient();
...
export const getRestaurantBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { cachedPublicQuery } = await import("./public-cache.server");
    return cachedPublicQuery(`getRestaurantBySlug:${data.slug}`, async () => {
      const { createPublicClient } = await import("./catalog.server");
      const supabase = createPublicClient();
